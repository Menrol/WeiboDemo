...
class HomeTableViewController: VisitorTableViewController {
    ...
    override func viewDidLoad() {
        ...
        // 注册通知
        NotificationCenter.default.addObserver(forName: NSNotification.Name.init(rawValue: WBStatusCellSelectedPcitureNotification), object: nil, queue: nil) { [weak self] (n) in
            
            guard let selectedIndex = n.userInfo?[WBStatusCellPhotoBrowserSelectedIndexKey] as? Int,
                let urls = n.userInfo?[WBStatusCellPhotoBrowserUrlsKey] as? [URL],
                let parentViews = n.userInfo?[WBStatusCellPhotoBrowserParentViewsKey] as? [UIImageView] else {
                return
            }
